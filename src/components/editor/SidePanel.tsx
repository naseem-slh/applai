      ...
      <details open={defaultOpen}>
        ...
        <summary
          className={cn(
            ...
            'rounded-card px-4 py-3 transition-colors select-none',
        ...
        >
          ...
          <span className="flex items-center gap-2">
            {badge}
            {/* Er dreht sich mit dem Zustand des `<details>` und ist
                `aria-hidden`: Was er sagt, sagt das Element selbst schon
                über `aria-expanded`. */}
            <CaretDownIcon className="shrink-0 text-[var(--accent-line)] transition-transform [details[open]_&]:rotate-180" />
          </span>
        </summary>
        <div className="border-t-2 border-[var(--line-soft)] px-4 py-4">{children}</div>
      </details>
    </Card>
  )
}
